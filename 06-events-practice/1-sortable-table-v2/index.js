export default class SortableTable {
  ...
  createTableHeader() {
    return ` 
            <div data-element="header" class="sortable-table__header sortable-table__row">
                ${this.tableHeader
                  .map((item) => this.createHeaderRow(item))
                  .join("")}
            </div>
        `;
  }
  ...
  createHeaderRow(obj) {
    return `
        <div class="sortable-table__cell" data-id="${obj.id}" data-sortable="${
      obj.sortable
    }">
            <span>${obj.title}</span>
  ...
  initEventListeners() {
    this.subElements.header.addEventListener("pointerdown", this.sortEvent.bind(this));
  }
